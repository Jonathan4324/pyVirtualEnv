...
def get_product_links(query, page_number=1):
  search_url = f"https://www.walmart.com/search?q={query}&page={page_number}"  #use "f string" to flip out search term and take whatever query

  response = requests.get(search_url, headers=HEADERS)

  soup = BeautifulSoup(response.text, "html.parser")

  links = soup.find_all('a', href=True)   #use this to find related links which might be sponsors
...
def main():
  OUTPUT_FILE = "product_info.jsonLink"

  with open(OUTPUT_FILE, 'w') as file: 
    page_number = 1
    while True:
      links = get_product_links("computer", page_number)
